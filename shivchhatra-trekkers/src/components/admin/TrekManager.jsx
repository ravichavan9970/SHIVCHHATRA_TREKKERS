...
export default function TrekManager() {
  const { treks, addTrek, updateTrek, deleteTrek, resetToDefaultTreks } = useTreks();
  const { getTrekTotalBookedSeats, getBatchBookedSeats } = useBookings();
  ...
  const handleSave = (e) => {
    ...
    if (currentTrek.id) {
      updateTrek(currentTrek.id, currentTrek);
    } else {
      addTrek(currentTrek);
    }
    setIsEditing(false);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h3 className="text-lg font-bold text-white font-heading">
            Trek Catalog & Departure Batches ({treks.length} Expeditions)
          </h3>
          <p className="text-xs text-slate-400">
            Configure fort dossiers, batch inventory limits, pricing, and boarding pickup points.
          </p>
        </div>

        <div className="flex items-center space-x-2">
          <button
            onClick={resetToDefaultTreks}
            className="px-3.5 py-2 bg-slate-900 hover:bg-slate-800 border border-slate-700 text-slate-300 hover:text-white text-xs font-semibold rounded-xl flex items-center space-x-1.5 transition-all"
            title="Reset to default fort list"
          >
          ...
          <button
            ...
            className="px-4 py-2 bg-gradient-to-r from-orange-600 to-amber-600 hover:from-orange-500 hover:to-amber-500 text-white text-xs font-bold rounded-xl shadow-lg shadow-orange-600/30 flex items-center space-x-1.5 transition-all cursor-pointer"
          >
            ...
            {treks.map((t) => (
              <tr key={t.id} className="hover:bg-slate-800/30 transition-colors">
                ...
                <td className="p-4 space-y-1">
                  {t.batches?.map((b) => {
                    const bookedLive = getBatchBookedSeats ? getBatchBookedSeats(t, b) : (b.bookedSeats || 0);
                    return (
            ...
            >
              <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                <h3 className="text-base font-bold text-white font-heading">
                  {currentTrek.id ? 'Edit Expedition Dossier' : 'Create New Expedition'}
                </h3>
                ...
                <div className="space-y-2 border-t border-slate-800 pt-3">
                  ...
                  {currentTrek.batches?.map((b) => (
                    <div key={b.id} className="grid grid-cols-4 gap-2 bg-slate-950 p-2.5 rounded-xl border border-slate-800 items-center">
                      ...
                      />
                      <div className="flex items-center justify-between">
                        <span className="text-[11px] text-orange-400 font-bold">{b.bookedSeats || 0} Booked</span>
                        <button
                          type="button"
